...
import (
	...
	netCommon "github.com/hashicorp/packer/common/net"
...
func (s *StepConfigureVNC) VNCAddress(ctx context.Context, vncBindAddress string, portMin, portMax int) (string, int, error) {
	var err error
	s.l, err = netCommon.ListenRangeConfig{
...
func (s *StepConfigureVNC) Run(ctx context.Context, state multistep.StateBag) multistep.StepAction {
	...
	if s.VNCOverWebsocket {
		...
		log.Printf("MOSS mks ticket %#v", ticket)
		host := ticket.Host
		if len(host) == 0 {
...
func (c *ConnWrapper) KeyEvent(keysym uint32, down bool) error {
	...
	log.Printf("MOSS reading back %#v", keysym)
	_, _, err := c.c.ReadMessage()
	if err != nil {
		return fmt.Errorf("Error reading back %#v: %s", keysym, err)
	}
...
func (c *ConnWrapper) handshake() error {
	mt, protocolVersion, err := c.c.ReadMessage()
	if err != nil {
		return fmt.Errorf("Error reading message protocol version: %s", err)
	}
	log.Printf("MOSS protocolVersion %s", string(protocolVersion))

	log.Printf("MOSS sending back protocol version")
	err = c.c.WriteMessage(mt, protocolVersion)
	if err != nil {
		return fmt.Errorf("Error reading message protocol version: %s", err)
	}

	log.Printf("MOSS reading security quantity")
	_, secQty, err := c.c.ReadMessage()
	if err != nil {
		return fmt.Errorf("Error reading security quantity: %s", err)
	}
	log.Printf("MOSS security quantity %s", string(secQty))

	log.Printf("MOSS sending security type")
	buf := new(bytes.Buffer)
	...
	if err := c.c.WriteMessage(websocket.BinaryMessage, buf.Bytes()); err != nil {
		return fmt.Errorf("Error sending security type: %s", err)
	}

	log.Printf("MOSS reading security type")
	_, serverInit, err := c.c.ReadMessage()
	if err != nil {
		return fmt.Errorf("Error reading security type: %s", err)
	}
	log.Printf("MOSS server init %s", string(serverInit))
